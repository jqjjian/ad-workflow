...
import {
    TikTokAccountApplication,
    TikTokAccountApplicationSchema,
    AuthRoleEnum,
...
import {
    tiktokApply,
    updateTikTokApply,
    getTikTokApplyRecord,
    bindExternalTaskId
} from '@/app/actions/workorder/tiktok-account-application'
...
export default function Page() {
    ...
    const [tiktokAccount, setTikTokAccount] =
        useState<TikTokAccountApplication>({
    ...
    const rule = createSchemaFieldRule(TikTokAccountApplicationSchema)
    ...
    const getDicData = async () => {
        try {
            ...
            // 获取TikTok时区字典
            const timezoneRes = await getTimezoneDictionary('TIKTOK')
        ...
        try {
            ...
            try {
                ...
                if (
                    ...
                    // 准备附件数据
                    const attachmentRecords = attachment.map(file => {
                        // 从上传结果中提取关键字段
                    ...
                    attachments.push(...attachmentRecords);
                }
            } catch (error) {
                console.error('附件处理错误:', error)
                message.error('附件处理错误，请稍后重试')
                setLoading(false)
                return
            }
            ...
            let res
            if (isEdit) {
                res = await updateTikTokApply(formData, userId, taskId)
            } else {
                res = await tiktokApply(formData, userId)
            }
    ...
    const fetchTaskDetail = async () => {
        ...
        setLoading(true)
        try {
            const response = await getTikTokApplyRecord(taskId)
            ...
            <ConfigProvider>
                <Breadcrumb
                    ...
                    items={[
                        ...
                        {
                            title: 'TikTok Ads ' + (isEdit ? '修改' : '开户')
                        }
                    ]}
                />
                <Title level={3} className="m-0 mb-4">
                    TikTok 平台{isEdit ? '修改申请' : '开户申请'}
                </Title>
                        ...
                        <Card className="mt-4">
                            ...
                            <Row gutter={[16, 16]} className="mt-4">
                                <Col span={12}>
                                    ...
                                    >
                                        <Input placeholder="请输入TikTok平台获取的任务ID" />
                                    </FormItem>
                                </Col>
