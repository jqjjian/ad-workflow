...
import {
    TikTokAccount,
    TikTokAccountSchema,
    TikTokAccountWithCompany
} from '@/schemas/tiktok-account'
...
function logDebug(message: string, data?: any) {
    Logger.debug(
        `[TikTokAccountAPI] ${message}${data ? ' ' + JSON.stringify(data) : ''}`
    )
    console.log(
        `[TikTokAccountAPI] ${message}`,
...
function logError(message: string, error: any) {
    ...
    console.error(`[TikTokAccountAPI] ${message}`, error)
}

export async function tiktokApply(
    data: TikTokAccountWithCompany,
    userId: string | undefined
): Promise<ApiResponse> {
    logDebug('开始处理TikTok账号申请', { userId })
    ...
    try {
        ...
        const validatedData = TikTokAccountSchema.parse(data)
        ...
        try {
            ...
            return await db.$transaction(async (tx) => {
                ...
                const workOrder = await tx.tecdo_work_orders.create({
                    data: {
                        ...
                        workOrderSubtype: WorkOrderSubtype.TIKTOK_ACCOUNT,
                ...
                await tx.tecdo_audit_logs.create({
                    data: {
                        entityType: 'TIKTOK_ACCOUNT_APPLICATION',
            ...
            logError('TikTok账号申请处理过程中发生错误', error)
            return ApiResponseBuilder.error(
                '1',
        ...
        logError('TikTok账号申请数据验证失败', error)
        return ApiResponseBuilder.error(
            '400',
...
export async function bindExternalTaskId(
    ...
    logDebug('开始绑定外部任务ID', { workOrderId, externalTaskId, userId })
    ...
    try {
        ...
        if (workOrder.status !== WorkOrderStatus.PENDING) {
            logDebug('工单状态不是待处理', { status: workOrder.status })
            return ApiResponseBuilder.error(
                '400',
        ...
        logError('绑定外部任务ID过程中发生错误', error)
        return ApiResponseBuilder.error(
            '1',
            error instanceof Error ? error.message : '未知错误'
        )
...
export async function updateTikTokApply(
    ...
    try {
        ...
        Logger.error(
            error instanceof Error ? error : new Error('TikTok账号申请修改失败')
        )
        return ApiResponseBuilder.error(
            '1',
            error instanceof Error ? error.message : '未知错误'
...
// 获取TikTok开户申请详情的方法
export async function getTikTokApplyRecord(
    taskId: string
): Promise<ApiResponse> {
    const session = await auth()
    if (!session) {
